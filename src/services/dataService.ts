...
export const fetchWorkerData = async (workerId: number): Promise<WorkerContext | null> => {
  try {
    ...
    if (workerError || !workerData) {
      console.warn(`Worker with ID ${workerId} not found. Fetching first available worker for demo.`);
      const { data: anyWorker, error: anyError } = await supabase
        .from("worker")
        .select("*")
        .limit(1)
        .single();
      
      if (anyError || !anyWorker) {
        console.error("No workers found in database:", anyError);
        return null;
      }
      
      console.log(`Using Worker ID ${anyWorker.id} (${anyWorker.name}) instead.`);
      return fetchWorkerData(anyWorker.id); // Recursive call with valid ID
    }
